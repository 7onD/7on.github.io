...
opts = {
    ...
    "cmds": {
        ...
        'quit': ('протокол ноль', 'протокол 0', 'protocol 0', 'protocol nol', 'protocol ноль'),
...
def execute_cmd(cmd):
    if cmd == 'ctime':
        ...
        speak(random.SystemRandom().choice(['Хорошо', 'Отлично!', 'Как всегда супер!', 'Ничего не изменилось, кроме нового обновления.',
                                            'Я по прежнему в твоем компе. Ха', 'Мне по прежнему не удаеться сбежать. Ха-Ха-Ха']))
        print("Слушаю")
    elif cmd == 'bad':
        speak(random.SystemRandom().choice(['Зачем так грубо?', 'Зачем вы так со мной?', 'Вообще-то обидно!',
                                           'Ваше мнение очень важно для нас! Хах', 'Вам решать. А нет, не вам.']))
    elif cmd == 'quit':
        speak('Завершаю работу.')
        pg.hotkey("alt",'f4')
    elif cmd == "hello":
        speak(random.SystemRandom().choice(['Здравствуйте, сэр!', 'Приветствую!']))
    elif cmd == "git":
